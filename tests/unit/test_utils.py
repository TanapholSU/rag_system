...
def test_get_filename_from_signed_url():
    """ "
    Test function to get filename from signed URL.
    The result should be identical to original filename.
    """
...
def test_get_filename_from_invalid_url():
    """ "
    Test function to get filename from signed URL.
    The result should be identical to original filename.
    """
    url = "http://127.0.0.1:9000/tektome/"
    with pytest.raises(ObjectStorageFileNotFoundError):
        result = get_filename_from_signed_url(url)


def test_is_allowed_content_type():
    """ "
    Test function to check MIME type.
    The supported MIME type should return True. Otherwise, False should be returned
    """
